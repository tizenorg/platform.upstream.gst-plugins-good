  ...
  while (traf_node) {
    ...
    while (uuid_node) {
      ...
      if (UUID_SAMPLE_ENCRYPT == uuid_type) {
        GST_ERROR_OBJECT (qtdemux, "found sample_encrypt UUID & decryption is not support");
        GST_ELEMENT_ERROR (qtdemux, STREAM, DECRYPT, ("decryption not supported"), (NULL));
  ...
  if (type->flags & QT_FLAG_CONTAINER) {
    ...
    switch (fourcc) {
      ...
      case FOURCC_uuid: {
        ...
        gst_byte_reader_init (&uuid_data, buffer, QT_UINT32 (buffer));
        uuid_type = qtdemux_get_uuid_type (qtdemux, &uuid_data, &uuid_offset);
        if ((UUID_PROTECTION_HEADER == uuid_type) || (UUID_SAMPLE_ENCRYPT == uuid_type)) {
          GST_ERROR_OBJECT (qtdemux, "found protection/sample_encrypt UUID & decryption is not support");
          GST_ELEMENT_ERROR (qtdemux, STREAM, DECRYPT, ("decryption not supported"), (NULL));
          return FALSE;
        } else {
          GST_WARNING_OBJECT (qtdemux, "Ignoring Wrong UUID...");
